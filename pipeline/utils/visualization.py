...
def get_city_base(CHICAGO_NEIGHBORHOOD):
    """ 
    ...
    """
    shapes = shp.Reader(CHICAGO_NEIGHBORHOOD).shapeRecords()
    x,y = [],[]
    for shape in shapes:
...
def plot_contour(kde_model):
    ...
    """
    xgrid = np.linspace(CHICAGO_COORDS['ll']['latitude'], CHICAGO_COORDS['ur']['longitude'], 195)
    ygrid = np.linspace(CHICAGO_COORDS['ll']['latitude'], CHICAGO_COORDS['ur']['longitude'], 210)
    ...
    levels = np.linspace(kde_values.min(), kde_values.max(), KDE_LEVELS)
...
def plot_scatter(data):
    ...
    city_x,city_y = get_city_base()
    fig = plt.figure(figsize = FIGURE_SIZE)
    plt.scatter(data['Longitude'],data['Latitude'], color = CONTOUR_PLOT_COLOUR, s = SCATTER_SIZE_OF_CRIME_POINTS)
    plt.scatter(city_x,city_y, color = CITY_MAP_COLOR, s = SCATTER_SIZE_OF_CHICAGO_CITY, zorder = CITY_MAP_ORDER)
